...
REM *********************************************************
REM 	Stimulated_echo_sweep.bat 
REM 	This file is used to run the program Stimulated_echo_sweep.exe. 
REM 	Details about the specific echo parameters are described below. 

REM 	Tycho Sleator.
REM 	2012/04/10 22:54:00
...
REM P3_TIME (microseconds): pulse time must be atleast 0.065. 
set P3_TIME=4.4
...
REM P3_PHASE (degrees): Output (Tx) phase of p3 pulse. 
set P3_PHASE=15.0

REM TAU1 (microseconds) must be at least 0.065.  
set TAU1=5000

REM TAU2_MIN (microseconds): Minimum value of tau. Must be at least 0.065.  
set TAU2_MIN=5000

REM DELTA_TAU2 (microseconds): Increment in tau.
set DELTA_TAU2=5000

REM N_TAU2 (must be integer > 0): Number of values of TAU.  Tau_i = Tau_min + i*DELTA_TAU for i = 0 to N_TAU-1 
set N_TAU2=10
...
Stimulated_echo_sweep %FILENAME% %SPECTROMETER_FREQUENCY% %SPECTRAL_WIDTH% %P1_TIME% %P2_TIME% %P3_TIME% %RINGDOWN_TIME% %P1_PHASE% %P2_PHASE% %P3_PHASE% %TAU1% %TAU2_MIN% %DELTA_TAU2% %N_TAU2% %NUMBER_OF_SCANS% %REPETITION_DELAY% %AMPLITUDE% %BLANKING_DELAY% %BYPASS_FIR% %ADC_FREQUENCY% %BLANKING_BIT% %DEBUG% %BOARD_NUMBER% 
pause
